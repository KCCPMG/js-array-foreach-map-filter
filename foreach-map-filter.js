...
function vowelCount(str){
    ...
    for (let vowel of ['a','e','i','o','u']) {
        let vowelCount = strArr.filter(ch => ch === vowel).length;
        if (vowelCount > 0) vowelObj[vowel] = vowelCount;
    }
    return vowelObj;
}
...
function find(arr, searchValue) {
    let found = undefined;
    if (arr.filter(el => el === searchValue).length > 0) found=searchValue;
    return found;
}
...
function removeVowels(str) {
    return str.toLowerCase().split("").filter(el => !el.match(/[aeiou]/)).join("");
}
...
function doubleOddNumbers(arr) {
    return arr.filter(el => el % 2 === 1).map(el => el*2);
}
